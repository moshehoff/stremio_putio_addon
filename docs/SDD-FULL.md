...
| **ארגון Put.io** | תיקיות לפי **שמות משתמשים**, קבצי torrent בפנים | סורקים **הכל**; שם התיקייה **לא** יוצר קטלוג נפרד |
| **קטלוג Stremio** | **אחד מאוחד** לכל החשבון | קטלוג יחיד `putio_series` + `putio_movies` (סרטים בשלב מאוחר) |
...
| **Credentials** | Put.io OAuth **token** קיים | M1: token ב-`.env` — בלי OAuth UI עד M7 |
...
2. **Parser סדרות** — קבצי torrent לרוב בפורmat `Show.S04E08.1080p...`; `parse-torrent-title` + regex `SxxExx` קריטיים ל-MVP.
3. **קטלוג מאוחד** — לא ממפים תיקיות משתמשים ל-catalogs; כל הקבצים נכנסים לאותו index.
...
| סדרה (meta) | `putio:series:{seriesKey}` | `putio:series:friends` |
| פרק (video) | `putio:series:{seriesKey}:{season}:{episode}` | `putio:series:friends:4:8` |
...
```json
{
  ...
  "types": ["movie", "series"],
  "catalogs": [
    { "type": "movie", "id": "putio_movies", "name": "Put.io Movies", "extra": [
      { "name": "search", "isRequired": false },
      { "name": "skip", "isRequired": false }
    ]},
    { "type": "series", "id": "putio_series", "name": "Put.io Series", "extra": [
      { "name": "search", "isRequired": false },
      { "name": "skip", "isRequired": false }
    ]}
  ],
...
**פורמט video ID לסדרות (Cinemeta convention):**
```
...
**זרימה ידנית:**
1. משתמש יוצר token ב-[app.put.io/oauth](https://app.put.io/oauth)
2. שם ב-`.env` → `PUTIO_TOKEN`
3. Worker/API משתמשים ב-token
...
User → GET /configure
     ...
     → encrypt & store in oauth_tokens table
     → redirect stremio://{BASE_URL}/{userSlug}/manifest.json
```

**OOB (חלופה ל-TV):**
```
GET /oauth/oob/start → display code + put.io/link
poll GET /oauth2/oob/code/{code} until token
```
...
### 8.5 Folder Mapping (אופציונלי M8)

```
User config: { "Movies": 12345, "TV": 67890 }
→ files under folder 12345 → catalog putio_movies
```

Put.io `parent_id` מאפשר mapping — **לא** להציג tree ב-UI.
...
| catalogId | SQL/filter |
|-----------|------------|
| `putio_movies` | `media.kind = 'movie' AND metadata_status = 'matched'` |
| `putio_series` | `media.kind = 'series'` (unique series) |
...
SDK: `{ metas, cacheMaxAge: 600 }` או Fastify `Cache-Control`.
...
| Key | TTL | Value |
|-----|-----|-------|
| `catalog:{userId}:{type}:{id}:{skip}:{search}` | 600s | JSON metas |
...
| M2.5 | CatalogService: `putio_series` | Returns MetaPreview[] (unique series) |
| M2.6 | Route `GET /catalog/series/putio_series.json` | Stremio shows series |
...
| # | משימה | Acceptance Criteria |
|---|--------|---------------------|
| M7.1 | `/configure` page | UI for connect |
| M7.2 | OAuth callback + token storage | Encrypted in DB |
| M7.3 | Per-user manifest URL | Multi-user works |
| M7.4 | Remove PUTIO_TOKEN env dependency | Production ready |
...
| `/catalog/movie/putio_movies.json` | `{ metas: [{ id, type, name, poster }] }` |
| `/catalog/movie/putio_movies/search=matrix.json` | filtered metas |
...
| `Friends.S04E08.mkv` | episode | meta: `putio:series:friends`, video: `putio:series:friends:4:8` |
